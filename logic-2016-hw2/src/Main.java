...
public class Main
{
    ...
    public static void main(String[] args) throws IOException {
        ...
        String exprS;
        int exprId;
        for (exprS = in.readLine(), exprId = 1; exprS != null; exprS = in.readLine(), ++exprId) {
    ...
    private static ArrayList<Expression> axioms = new ArrayList<Expression>() {{
        try {
            ...
            while ((axiom = axiomsReader.readLine()) != null) {
                ...
                System.out.println(axiom + " Parsed");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    static void parseHyppothethis() throws IOException {
        ...
        if (indexOfHypEnd == -1) {
            ...
            l = d.substring(0, indexOfHypEnd) + ",";
            r = d.substring(indexOfHypEnd + 2, d.length());
            boolean flag = false;
            while (true) {
    ...
    private static boolean isExistAxiom(Expression expr) {
        if (expr.instance == '>') {
            Expression lhs = ((BinaryOperation) expr).lhs; // Phi[x:=theta]
            Expression rhs = ((BinaryOperation) expr).rhs; // ?x Phi
            if (rhs.instance == '?') {
    ...
    private static boolean isForAllAxiom(Expression expr) {
        ...
        if (expr.instance == '>') {
            Expression lhs = ((BinaryOperation) expr).lhs; // @x Phi
            Expression rhs = ((BinaryOperation) expr).rhs; // Phi[x := theta]
            if (lhs.instance == '@') {
    ...
    private static boolean isRightSubstitutionMade(Expression phi, Expression rhs, Variable x, Expression dvalue) {
        ...
        System.out.println("\t" + phi + "|" + rhs + "|var=" + x);
    ...
    private static boolean isRightSubstitutionMadeHelper(Expression phi, Expression rhs, Variable x, Multiset<String> nonFreeVars) {
        ...
        if (phi.instance == Variable.VAR && !nonFreeVars.contains(phi.toString())) {
            if (d == null) {
                getFreeVars(rhs, substitutionHelperSetForFreeVars, HashMultiset.create());
                d = rhs;
            } else if (!phi.equals(d)) {
                return false;
            }
